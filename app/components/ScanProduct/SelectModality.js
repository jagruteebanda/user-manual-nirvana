...
class SelectModality extends Component {
  ...
  getModalities = () => {
    ...
    const url = `${window.UserManualNirvana.basePath}/User/${
      userDetails['user']['id']
    }/Modality`;
  ...
  handleModalityChange = modalityName => {
    let selectedModality = this.state.modalities.filter(
    ...
    const url = `${window.UserManualNirvana.basePath}/Modality/${
      selectedModality[0].id
    }/Product`;
    http.get(url, null, (err, res) => {
      if (err) {
        ToastAndroid.show(
          'Error getting products by modality',
          ToastAndroid.SHORT,
        );
      }
      if (res) {
        this.setState({
          productsByModality: res.docs.filter(product => !product['isDeleted']),
        });
      }
    });
  };
  ...
  handleProductChange = productName => {
    let selectedProduct = this.state.productsByModality.filter(
  ...
  handleStartGeneratingManual = () => {
    if (this.state.selectedModality === 'Select modality') {
      this.setState({
        noModalitySelectedError: 'Please select a modality from the list.',
      });
      return;
    }
    ...
    if (this.state.selectedProduct === 'Select product') {
      ...
      http.post(url, null, body, (err, res) => {
        this.setState({
          loading: false,
        });
        if (err) {
          ToastAndroid.show(
            'There was error saving thr product!',
            ToastAndroid.SHORT,
          );
        }
        if (res) {
      ...
      <View style={{flex: 1, width, height}}>
        ...
        {this.state.loading ? (
          ...
          <View style={{flexDirection: 'column', width, height: height - 50}}>
            <View
              style={{height: 50, paddingLeft: 16, justifyContent: 'center'}}>
            ...
            <View
              ...
              <Picker
                ...
                onValueChange={(itemValue, itemIndex) =>
                  this.handleModalityChange(itemValue)
                }>
                <Picker.Item
                  style={{fontFamily: 'SourceSansPro-Light', fontSize: 16}}
                  label={this.state.selectedModality}
                  value={this.state.selectedModality}
                />
                {this.state.modalities &&
            ...
            <View
              ...
              <Picker
                ...
                />
                {this.state.productsByModality &&
                  this.state.productsByModality.map((item, i) => (
                    <Picker.Item
                      style={{fontFamily: 'SourceSansPro-Light', fontSize: 16}}
                      key={`productItem_${i}`}
                      label={item.name}
            ...
            <TouchableOpacity
              onPress={() => this.handleStartGeneratingManual()}>
              <View
                style={{
                  flexDirection: 'row',
                  width: width - 32,
