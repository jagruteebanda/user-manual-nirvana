...
import { View, Dimensions, Text, TouchableOpacity, ToastAndroid, AsyncStorage } from "react-native";
...
export default class HTMLViewScreen extends Component {
      ...
      checkForUploadedTaskContent = async () => {
            try {
                  let taskContentUploadDetails = JSON.parse(await AsyncStorage.getItem('taskContentUploadDetails'));
                  if (taskContentUploadDetails !== null && taskContentUploadDetails.length > 0) {
                        let isTaskUploaded = taskContentUploadDetails.filter((taskContent) => taskContent.taskName === this.props.navigation.state.params.taskData.taskName);
                        if (isTaskUploaded.length > 0)
                              this.props.navigation.state.params.taskData.task['contentId'] = isTaskUploaded[0].contentId
                  }
            } catch (err) {
      ...
      uploadTaskContent = async () => {
            ...
            // http.post("http://localhost:3000/apis/html/upload", null, payload, async (err, res) => {
            //       if (err) {
            //             ToastAndroid.show("There was error uploading the task!", ToastAndroid.SHORT);
            //       }
            //       if (res) {
            try {
                  ...
                  let taskContentUploadDetails = await AsyncStorage.getItem('taskContentUploadDetails');
                  ...
                  AsyncStorage.setItem('taskContentUploadDetails', JSON.stringify(taskContentUploadDetails));
                  this.props.navigation.navigate('UploadTask', {
                        taskData: taskData.task
                  });
            } catch (err) {
                  ToastAndroid.show("Error in saving task details!", ToastAndroid.SHORT);
            }
            //       }
            // });
      }
